...
module.exports = function (options) {
    ...
    return function buildProduction(assetGraph, cb) {
        ...
        assetGraph
            ...
            .queue(function countNumberOfCssRules(assetGraph) {
                assetGraph.findAssets({isLoaded: true, type: 'Css'}).forEach(function (cssAsset) {
                    var count = 0;
                    cssAsset._parseTree.cssRules.forEach(function (rule) {
                        if (rule.selectorText) {
                            count += rule.selectorText.split(',').length;
                        }
                    });
                    if (count > 4095) {
                        console.warn('WARNING: Old IE CSS rule limit reached. ' + count + ' total rules, ' + (count - 4096) + ' will be ignored by IE9 and below.');
                    }
                });
            })
